...
public class User {
	...
	private static final String PROMOTER_MERCHANT_FIELD = "MerchentName";
	...
	// public void
	// setOnParseLoginSucceedListener(onParseLoginStateChangeListener listener)
	// {
	// mLoginStateChangeListener = listener;
	// }
	// public void setOnParseLoginFailureListener(onParseLoginFailureListener
	// listener)
	// {
	...
	public void checkTrustRequests() {
		...
		ParseQuery<TrustRequest> query = ParseQuery
				.getQuery(TrustRequest.class);
		...
		query.findInBackground(new FindCallback<TrustRequest>() {
			public void done(List<TrustRequest> trustReq, ParseException e) {
				...
				if (e == null) {
					if (trustReq.size() > 0) {
						Log.d(TAG, String.format("request arrived from %s",
								trustReq.get(0).getRequestFrom()));
	...
	public static ParseUser findUserByEmail(final String email) {
		...
		try {
			...
			if (users.size() > 0) {
				Log.v(TAG, String.format("found user by email %s", users.get(0)
						.getUsername()));
				return users.get(0);
			}
		...
		ParseUser.requestPasswordResetInBackground(email,
				new RequestPasswordResetCallback() {
					public void done(ParseException e) {
						if (e == null) {
							Log.v(TAG,
									"Reset password succeeded - An email was successfully sent with reset instructions");
						} else {
							Log.v(TAG,
									String.format("Reset password failed: %s",
											e.getMessage()));
						}
					}
				});
	...
	public static void findUserByEmailAsync(final String email) {
		...
		query.findInBackground(new FindCallback<ParseUser>() {
			...
			public void done(List<ParseUser> objects, ParseException e) {
				if (e == null) {
					Log.v(TAG, String.format("found user by email %s", objects
							.get(0).getUsername()));

				} else {
					Log.v(TAG,
							String.format("Query user failed: %s",
									e.getMessage()));
				}
			}
		});
		...
		if (isLoggedIn()) {
			...
			mUser.refreshInBackground(new RefreshCallback() {
				...
				public void done(ParseObject arg0, ParseException e) {
					if (e != null) {
						Utils.sendMessage(WePromoteApplication.getContext(),
								new InternalMessage(
										InternalMessage.MESSAGE_LOGIN_FAILURE,
										e.getMessage()));
					} else {
						checkEmailVerified();
	...
	public void attemptLogin() {
		...
		if (isLoggedIn()) {
			...
			WePromoteApplication.setInstallationParams(getPromoterID(false),
					isFacebookUser());
			checkEmailVerified();
	...
	public static String checkFacebookUser(String username) {
		SharedPreferences facebookUsers = WePromoteApplication.getContext()
				.getSharedPreferences(FACEBOOK_USERS_EMAIL_PREFS,
		...
		ParseUser.logInInBackground(userName, pass, new LogInCallback() {
			public void done(ParseUser user, ParseException e) {
				if (user != null) {
					mUser = user;
					Log.v(TAG,
							String.format("Login succeeded: %s",
									user.getUsername()));
					checkEmailVerified();
					WePromoteApplication.setInstallationParams(
							getPromoterID(false), isFacebookUser());
					Utils.sendMessage(WePromoteApplication.getContext(),
							new InternalMessage(
									InternalMessage.MESSAGE_LOGIN_SUCCESSFUL,
									null));
					fireOnLoginStateChange();
				} else {
					Log.e(TAG,
							String.format("login failed with error: %s",
									e.getMessage()));
					Utils.sendMessage(
							WePromoteApplication.getContext(),
							new InternalMessage(
									InternalMessage.MESSAGE_LOGIN_FAILURE, e
											.getMessage()));
					Utils.sendMessage(
							WePromoteApplication.getContext(),
							new InternalMessage(
									InternalMessage.MESSAGE_SHOW_SETUP_WIZARD_SPINNER,
									"false"));
				}
			}
		});
	...
	public void loginFacebook(final Activity context) {
		List<String> permissions = Arrays.asList("basic_info",
				// "public_profile" - public_profile should replace basic_info
				// when facebook app is not installed. but we prevent this since
				// other stuff won't work if using the web login for facebook
			...
			public void done(final ParseUser facebookUser, ParseException e) {
				if (e != null) {
					...
					Utils.sendMessage(
							WePromoteApplication.getContext(),
							new InternalMessage(
									InternalMessage.MESSAGE_FACEBOOK_LOGIN_FAILURE,
									e.getMessage()));
				...
				if (facebookUser == null) {
					Log.d(TAG,
							"facebookUser returned null. might be due to cancellation of Facebook login by user.");
				} else if (facebookUser.isNew()) {
					Log.d(TAG, "User signed up and logged in through Facebook!");
					mUser = facebookUser;
					// requestFacebookEmailPermissions(context,
					// facebookSession);
					getFacebookEmail();
					Log.d(TAG,
							String.format("User logged in via Facebook!, resolving email in progress..."));
					fireOnLoginStateChange();
				} else {
	...
	private void fireOnLoginStateChange() {
		Utils.sendMessage(WePromoteApplication.getContext(),
				new InternalMessage(InternalMessage.MESSAGE_LOGIN_STATE_CHANGE,
						null));
	}
	...
	private void getFacebookEmail() {
		...
		Request request = new Request(session, "/fql", params, HttpMethod.GET,
				onFacebookPersonalInfoReceive);
		Request.executeBatchAsync(request);
	}

	// private void requestFacebookEmailPermissions(final Activity
	// context,Session facebookSession) {
	...
	// Session.NewPermissionsRequest(context, permissions);
	...
	public boolean hasFacebookPublishPermission(Session facebookSession) {
		if (isFacebookUser()) {
			return facebookSession.getPermissions().contains(
					FACEBOOK_PERMISSION_PUBLISH_ACTIONS);
		} else
			return false;
	}
	...
	private Request.Callback onFacebookPersonalInfoReceive = new Request.Callback() {
				...
				for (int i = 0; i < (arr.length()); i++) {
					...
					if (email != null && email.trim().length() > 0) {
						...
						Utils.sendMessage(
								WePromoteApplication.getContext(),
								new InternalMessage(
										InternalMessage.MESSAGE_FACEBOOK_EMAIL_RESOLVED,
										null));
					}
				}
			} catch (Throwable t) {
		...
		private void saveFacebookEmail(String email) {
			...
			SharedPreferences facebookUsersPrefs = WePromoteApplication
					.getContext().getSharedPreferences(
							FACEBOOK_USERS_EMAIL_PREFS, Context.MODE_PRIVATE);
			SharedPreferences.Editor editor;
			editor = facebookUsersPrefs.edit();
			editor.putString(mUser.getUsername(), email);
			editor.commit();
			Log.d(TAG, String.format("Resolved user email via facebook. %s",
					email));
		}
	};

	public String getMerchantName() {
		if (mUser == null || mUser.get(PROMOTER_MERCHANT_FIELD) == null)
			return null;

		return (String) mUser.get(PROMOTER_MERCHANT_FIELD);
		...
		ParseCloud.callFunctionInBackground("hello",
				new HashMap<String, Object>(), new FunctionCallback<String>() {
					public void done(String result, ParseException e) {
						...
						Log.v(Constants.TAG, "cloud hello world result: "
								+ result);
					}
				});
	}

	private void createTrustedUser(String trustedEmail, boolean isConfirmed,
			boolean isRequestedByMe) {
		...
		mUser.saveInBackground(new SaveCallback() {
			...
			public void done(ParseException arg0) {
				Utils.sendMessage(
						WePromoteApplication.getContext(),
						new InternalMessage(
								InternalMessage.MESSAGE_PROMOTER_ID_SAVED, null));
			...
			public void done(ParseException e) {
				if (e == null) {
					...
					Utils.sendMessage(
							WePromoteApplication.getContext(),
							new InternalMessage(
									InternalMessage.MESSAGE_SIGNUP_FAILURE, e
											.getMessage()));
					Utils.sendMessage(
							WePromoteApplication.getContext(),
							new InternalMessage(
